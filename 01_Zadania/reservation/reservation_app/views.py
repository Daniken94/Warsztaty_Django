...
class AddRoomView(View):
    ...
    def post(self, request):
        ...
        if capacity <= 0:
            return render(request, "add_room.html", context={"error": "Pojemność sali nie może być mniejsza niż 1!"})
...
class ModifyRoomView(View):
    ...
    def post(self, request, room_id):
        ...
        if capacity <= 0:
            return render(request, "modify_room.html", context={"room": room,
                                                                "error": "Pojemność sali musi być liczbą dodatnią!"})
...
class ReservationView(View):
    ...
    def post(self, request, room_id):
        ...
        if RoomReservation.objects.filter(room=room, date=date):
            return render(request, "reservation.html", context={"room": room, "reservations": reservations, "error": "Sala jest juz zarezerwowana!"})


        if date < str(datetime.date.today()):
            return render(request, "reservation.html", context={"room": room, "reservations": reservations, "error": "Nieprawidlowa data!"})
